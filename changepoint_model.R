...
fit_chunk_non_memoized = function(ts_matrix, x, start, end, 
                                  weights, ...) {
  # Weights average to 1, & are proportional to total rodents caught that month
  m = multinom(
    ts_matrix ~ sin_year + cos_year, 
    data = x,
    maxit = 1E5,
    weights = weights,
...
get_ll_non_memoized = function(ts_matrix, x, changepoints, 
                               weights, ...){
  ...
  sum(
    sapply(
      ...
      function(i){
        fit_chunk(ts_matrix, x, changedates[i], changedates[i + 1], 
                  weights = weights, ...)
      }
    )
  )
...
#' 
#' @param ts_matrix matrix of population/community time series (gamma from LDA object)
...
#' 
#' 
get_ll_non_memoized_plot = function(ts_matrix, x, changepoints, weights, ...){
  
  changedates = c(-Inf, x$year_continuous[changepoints], Inf)
  all_sections = data.frame()
  for (i in seq(length(changedates)-1)) {
    section = fit_section(ts_matrix, x, changedates[i], changedates[i+1], weights = weights)
...
#' @param weights same weights used in changepoint_model()
#' 
fit_section = function(ts_matrix, x, start, end, weights, ...) {
  m = multinom(
    ts_matrix ~ sin_year + cos_year, 
    data = x,
    maxit = 1E5,
...
#' Model to locate given number of change points given a time series matrix
#' 
#' @param ts_matrix matrix of population/community metrics
#' @param x time data frame. contains column year_continuous, sin_year, cos_year
...
changepoint_model = function(ts_matrix,
                             x,
                             n_changepoints,
                             N_temps = 6,
                             maxit = 1E5,
  ...
  lls = sapply(1:N_temps, 
               function(j){get_ll(ts_matrix, x, changepoints[ , j], 
                                  weights = weights)})
  ...
  for (i in 1:maxit) {
    ...
    proposed_lls = sapply(1:N_temps, function(j){get_ll(ts_matrix, x, proposed_changepoints[ , j],
                                                        weights = weights)})
